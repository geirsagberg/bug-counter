...
import type { BugCounterData, BugRegistration, EnvironmentDefinition } from "./types";

type AppProps = { repository: BugRepository };
...
function initialSinceDate() {
  return window.localStorage.getItem(SINCE_DATE_KEY) || formatLocalDate(new Date());
}
  ...
  return (
    <main className="shell">
      <header className="summary-heading">
        <h1>
          <strong>{filtered.length}</strong> {filtered.length === 1 ? "bug" : "bugs"} since
          <label className="date-picker">
            <span className={since ? undefined : "date-placeholder"} aria-hidden="true">
              {since || "YYYY-MM-DD"}
            </span>
            <input
              type="date"
      ...
      <section className="environment-list" aria-label="Bug counts by environment and severity">
        {data.environments.map((environmentDefinition) => (
          <EnvironmentSummary
            ...
            onColorChange={(color) => updateEnvironmentColor(environmentDefinition.name, color)}
            onDelete={() => setDeletingEnvironment(environmentDefinition.name)}
          />
        ))}
      </section>
          ...
          <form className="entry-form" onSubmit={register}>
            <label>
              ...
              >
                ...
                {data.severities.map((value) => <option value={value} key={value}>{value}</option>)}
              </select>
            </label>
            <label>
              ...
              <datalist id="environment-options">
                {data.environments.map((value) => <option value={value.name} key={value.name} />)}
              </datalist>
            </label>
            ...
            <div className="form-actions">
              ...
              <button className="quiet-button" type="button" onClick={() => setIsAdding(false)}>Cancel</button>
            </div>
          </form>
        )}
      </section>
      ...
      {data.registrations.length > 0 && (
        <details className="registrations">
          <summary>Manage registrations <span>{data.registrations.length}</span></summary>
          <ol>
            {data.registrations.map((bug) => (
              <li key={bug.id}>
                <div>
                  ...
                  {bug.description && <span className="bug-description">{bug.description}</span>}
                </div>
                <time dateTime={bug.registeredAt}>{formatTimestamp(bug.registeredAt)}</time>
                <button type="button" onClick={() => deleteRegistration(bug)}>Delete</button>
              </li>
            ))}
          </ol>
        </details>
      ...
      {isResetting && (
        <div className="dialog-backdrop" role="presentation" onMouseDown={() => setIsResetting(false)}>
          <section className="dialog" role="alertdialog" aria-modal="true" aria-labelledby="reset-title" onMouseDown={(event) => event.stopPropagation()}>
            <h2 id="reset-title">Reset all registrations?</h2>
            <p>Your environments will stay.</p>
            <div className="dialog-actions">
              <button className="quiet-button" type="button" onClick={() => setIsResetting(false)}>Cancel</button>
              <button className="danger-button" type="button" onClick={reset}>Reset all</button>
            </div>
          </section>
        </div>
      )}
      ...
      {deletingEnvironment && (
        <div className="dialog-backdrop" role="presentation" onMouseDown={() => setDeletingEnvironment(null)}>
          <section className="dialog" role="alertdialog" aria-modal="true" aria-labelledby="delete-environment-title" onMouseDown={(event) => event.stopPropagation()}>
            <h2 id="delete-environment-title">Delete {deletingEnvironment}?</h2>
            <p>All bugs registered in this environment will be removed.</p>
            <div className="dialog-actions">
              <button className="quiet-button" type="button" onClick={() => setDeletingEnvironment(null)}>Cancel</button>
              <button className="danger-button" type="button" onClick={deleteEnvironment}>Delete environment</button>
            </div>
          </section>
        </div>
      )}
...
function EnvironmentSummary({
  ...
  return (
    <article className="environment-row" style={{ "--environment-color": color } as CSSProperties}>
      <div className="environment-name">
        ...
        <div className="environment-actions">
          ...
          />
          <button type="button" onClick={onDelete} aria-label={`Delete ${name} environment`}>Delete</button>
        </div>
      </div>
      <dl>
